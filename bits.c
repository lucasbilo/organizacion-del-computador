...
int addOK(int x, int y) {
  ...
  int signX = x >> 31; // lo mismo con x e y
  int signY = y >> 31;
...
int isGreater(int x, int y) {
  // diferenteSignoXY & signoY -> son diferentes e y es menor que 0
  // !(diferenteSignoXY | signoDiferencia) -> 1 si tienen mismo signo y la diferencia es positiva
  int xSign = x >> 31;
  int ySign = y >> 31;
  int negativeY = ~y + 1;
